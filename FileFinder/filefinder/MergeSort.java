...
public class MergeSort {
    ...
    public static void sortWithThread(int[] randomNumbers) throws InterruptedException {
        if (randomNumbers.length > 1) {
            int[] firstHalf = Arrays.copyOf(randomNumbers, randomNumbers.length / 2);
            int[] secondHalf = Arrays.copyOfRange(randomNumbers, randomNumbers.length / 2, randomNumbers.length);

            if (randomNumbers.length < THRESHOLD) {
                sort(randomNumbers);

            } else {
                MergeThread half1 = new MergeThread(firstHalf);
                MergeThread half2 = new MergeThread(secondHalf);
                ...
                merge(firstHalf, secondHalf, randomNumbers);
